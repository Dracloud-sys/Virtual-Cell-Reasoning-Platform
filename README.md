...
- **Deterministic verification gate** (PR8d-1): opt-in (`verify: true`, requires `extract: true`) re-checks retained candidates against the current document and records one decision each — only an exact, quantitative **table** measurement is machine-verified; prose, claims, author interpretations, statistics and unparsed values are pending human review; source-integrity failures are rejected. Canonical conversion (PR8d-2) and KnowledgeStore ingestion are later slices; nothing here is written to the graph.
