...
- **Automated literature discovery** (PR8b): `virtualcell literature discover` finds external papers via the Europe PMC public API, deduplicates them, and returns metadata + a transparent relevance breakdown as a typed bundle — **not** biological claims. Extraction/verification of quantitative evidence (and conversion to canonical runs) are the next slices; discovery never writes to the knowledge graph.
